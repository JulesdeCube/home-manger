{
    ...
    aliases = {
        ...
        stat = "log --stat --decorate --format=format:'%C(bold blue)%h %C(bold red)<%an> %C(bold green)(%ad)%C(reset)%n%C(bold yellow)%D%C(reset)%n%C(bold white)%s%n%n%C(reset)%C(white)%b%C(reset)'";
